...
% Constructing the channels
   %AoD_el(1:2,:)=pi*rand(1,Num_paths)-pi/2;
   % AoD_el(3:4,:)=AoD_el(1:2,:)+pi/2;
  ...
  %  if u<=(Num_users)
         AoD_el(u,:)=2*pi*rand(1,Num_paths)-pi/2;       
         AoD_az(u,:)=2*pi*rand(1,Num_paths);
%      elseif u== (Num_users)
%         AoD_el(u,:)=pi/2 + 0.2*pi*rand(1,Num_paths);
%         AoD_az(u,:)=pi/2+0.2*pi*rand(1,Num_paths);
%     elseif u ==(Num_users-1)
%         AoD_el(u,:)=pi/2 + 0.2*pi*rand(1,Num_paths);
%         AoD_az(u,:)=pi*rand(1,Num_paths);
 %    end
%          if u == Num_users
%         AoD_el(u,:)=0.2*pi*rand(1,Num_paths);
%         AoD_az(u,:)=pi/2+0.2*pi*rand(1,Num_paths);
%         end
%     if u== Num_users-1
%         AoD_el(u,:)=pi/2+0.2*pi*rand(1,Num_paths);
...
%     end
%     if u==Num_users
%          AoD_el(u,:)=0.1*2*pi*rand(1,Num_paths);
