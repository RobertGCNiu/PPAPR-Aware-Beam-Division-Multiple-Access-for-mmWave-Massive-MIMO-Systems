...
function [Frf_selected,  H_selected, Wrf_selected]= greedyAlgorithm(H, a_TX, a_RX, users, usersToSupport, Space_have)
%%%%%%%%%%%%%%%%%%%
...
%             end
%           else
                k_rank = randperm(usersToSupport);
              Frf_selected_all_user(:,1) = st(:, k_rank(1));
%           end
            users_selected = k_rank(1);
  while length(users_selected)<(usersToSupport)
...
% u_selected = optimal_user_collection(:, optimal_user_for_select);
 H_selected = H(users_selected, :,:);
 Frf_selected(:,:) =  Frf_selected_all_user;
 Wrf_selected = a_RX(:, users_selected);
